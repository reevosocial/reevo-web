...
//	$field_icon.addClass('hidden fa-pulse fa-spinner').attr('title', '');
	$field_icon.addClass('fa-spinner').attr('title', '');
		...
		success: function(data) {
			...
			if (data.output) {
				...
				if (data.output.status === true) {
					...
					$field_icon.prop('innerHTML','');

				}

				if (data.output.text) {
					$field_icon.prop('innerHTML',data.output.text);
	...
	$(document).on('keyup', '.elgg-form-register input[name="password"], .elgg-form-register input[name="password2"]', function(event) {
		...
		if ((password1 !== '') && (password2 !== '')) {
			...
			if (password1 == password2) {
				$field_icon.addClass('fa-check-circle');
				$field.removeClass('profile_manager_register_missing');
				$field_icon.prop('innerHTML',elgg.echo(''));

				$submit_button.prop('disabled', false);
				$submit_button.removeClass('button-disabled');
			} else {
				$field_icon.addClass('fa-exclamation-circle').attr('title', elgg.echo('RegistrationException:PasswordMismatch'));
				$field_icon.prop('innerHTML',elgg.echo('RegistrationException:PasswordMismatch'));
