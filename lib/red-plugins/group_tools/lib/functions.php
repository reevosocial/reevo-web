...
function group_tools_blogadmin_access($group_guid) {
	...
	if ($entity->type == 'group') {
	  if ($entity->getPrivateSetting("{$prefix}blogprivate") == 'yes') {
	    $owner = $entity->owner_guid;
	    array_push($valid_users,$owner);
	    $admins = elgg_get_entities_from_relationship([ 'relationship' => 'group_admin', 'relationship_guid' => $group_guid, 'inverse_relationship' => true,]);
	    foreach ($admins as $key => $value) {
	      array_push($valid_users,$value->guid);
	    }
	    if (in_array(elgg_get_logged_in_user_guid(), $valid_users))
	    {
				// El usuario puede crear contenidos en el blog
				$result = 1;
	    } else {
				// El usuario no puede crear contenidos en el blog
				$result = 0;
			}
	  } else {
			$result = 1;
