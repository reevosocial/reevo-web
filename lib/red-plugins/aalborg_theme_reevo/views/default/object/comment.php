...
HTML;
	...
	$excerpt = elgg_get_excerpt($comment->description, 300);
	// $posted = elgg_echo('generic_comment:on', array($commenter_link, $entity_link));
  $posted = $commenter_link;
  $entity_title = elgg_echo('aalborg_theme_reevo:continue');
  if ($excerpt != $comment->description) {
    $more = "<a href=\"{$entity->getURL()}\">$entity_title</a>";
  }


	$body = <<<HTML
<span class="elgg-subtext">
	$posted ($friendlytime): $excerpt $more
</span>
HTML;
