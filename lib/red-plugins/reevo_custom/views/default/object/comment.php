...
// Customiza el estilo de los comentarios para los eventos, porque los usamos para publicar novedades del evento.
...
switch ($subtype) {
	case 'event': // muestra el comentario como novedad del evento
		$commenter = get_user($comment->owner_guid);
		if (!$entity || !$commenter) {
			return true;
		}

		$friendlytime = gmdate("d-m-Y H:i", $comment->time_created);
