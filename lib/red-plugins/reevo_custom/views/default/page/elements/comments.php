...
// muestra la caja de comentario solo para el creador del evento, organizadores y admins del sitio
...
if ($show_add_form) {
  ...
  if ($subtype != 'event' ) {
	...
	} else { // muestra la caja de comentario solo para el creador del evento, organizadores y admins del sitio
		$user = elgg_get_logged_in_user_entity();
		$admin =elgg_is_admin_logged_in();
		$organizer_guids = $vars['entity']->organizer_guids;


		if ($vars['entity']->owner_guid == $user->guid || $admin || in_array($user->guid, $organizer_guids)) {
			$content .= elgg_view_form('comment/save', array(), $vars);
		}
	}
}
