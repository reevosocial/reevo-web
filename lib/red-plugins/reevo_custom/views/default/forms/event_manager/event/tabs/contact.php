...
global $fbevent;
if (!empty($fbevent)) {
	$organizer = $fbevent['owner']['name'];
	// Intenta obtener un email que haya en la descripción del evento
	preg_match_all("/[\._a-zA-Z0-9-]+@[\._a-zA-Z0-9-]+/i", $fbevent['description'], $emails);
	$contact_details = $emails[0];
	$website = 'http://facebook.com/profile.php?id=' . $fbevent['id'];
} else {
